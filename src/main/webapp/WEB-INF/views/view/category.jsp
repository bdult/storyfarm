...
<form id="playListForm" action="playList.do" method="post">        
	...
	<div class="paginate_complex">
		<c:if test="${ not empty pageLink.pagePrev }">
			<a class="direction prev" href="${ contextPath }/category.do?cate_id=${ cateDetail.CATE_ID }&pageNum=${ pageLink.pagePrev }${ empty search.search? '' : search.search }"><img src="${ contextPath }/assets/images/common/btn_pagingPre_off.gif"  alt="이전 페이지 이동"  class="rollimg"></a>
		</c:if>
		...
		<c:if test="${ not empty pageLink.pageNext }">
			<a class="direction next" href="${ contextPath }/category.do?cate_id=${ cateDetail.CATE_ID }&pageNum=${ pageLink.pageNext }${ empty search.search? '' : search.search }"><img src="${ contextPath }/assets/images/common/btn_pagingNext_off.gif" alt="다음 페이지 이동"  class="rollimg"></a>
		</c:if>
    </div>
	</c:if>
	<!--//paginate -->

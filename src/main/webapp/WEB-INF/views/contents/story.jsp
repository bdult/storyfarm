...
<div id="content">
	...
	<form id="playListForm" action="playList.do" method="post">
		<h2 class="divTit">
			<img src="${ contextPath }/assets/images/content/title_fairytale.gif" alt="오즈월드 동화">
		</h2>
		<!-- location -->
		<div id="divLocation">
			<ul class="location">
				<li class="first"><a href="${ contextPath }" class="home"><img src="${ contextPath }/assets/images/common/blt_home.gif" alt="home"></a></li>
				<li class="current">동화</li>
			</ul>
		</div>
		<!-- //location -->

		<div class="topBanner">
			<img src="${ contextPath }/assets/images/content/fairytale_Timg.jpg" alt="오즈월드 동화">
		</div>

		<!-- conLst -->
		<div class="conLst story">
			<ul>
					...
					<c:forEach items="${ pageLink.pageList }" var="page">
						<c:choose>
							<c:when test="${ page.pageNum eq pageLink.pageNum }">
								<strong class="story">${ page.pageNum }</strong>
							</c:when>
							<c:otherwise>

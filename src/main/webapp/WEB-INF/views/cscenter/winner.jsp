...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
				...
				<!-- //location -->
              	...
              	<div class="tblHeadSch">
                	<input name="search" id="search-input" type="text" value="${ search }" class="input"><a href="#"><img src="../assets/images/common/btn_search2.gif" alt="검색" class="bt"></a>
                </div>
				...
				<div class="paginate_complex">
					<c:if test="${ not empty pageLink }">
						...
						<a class="direction prev" href="${ contextPath }/cscenter/winner.do?pageNum=${ pageLink.pagePrev }${ empty search.search? '' : search.search }"><img src="${ contextPath }/assets/images/common/btn_pagingPre_off.gif"  alt="이전 페이지 이동"  class="rollimg"></a>
						</c:if>
						<c:forEach items="${ pageLink.pageList }" var="page">
							<c:choose>
								...
								<c:otherwise>
									<a href="${ contextPath }/cscenter/winner.do?pageNum=${ page.pageNum }${ empty search.search? '' : search.search }&search=${ searchList.search }">${ page.pageNum }</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${ not empty pageLink.pageNext }">
						<a class="direction next" href="${ contextPath }/cscenter/winner.do?pageNum=${ pageLink.pageNext }${ empty search.search? '' : search.search }"><img src="${ contextPath }/assets/images/common/btn_pagingNext_off.gif" alt="다음 페이지 이동"  class="rollimg"></a>
						</c:if>
					</c:if>
				</div>
				<!--//paginate -->
